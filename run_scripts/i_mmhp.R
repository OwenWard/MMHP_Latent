...
immhp_model <- cmdstan_model("lib/I-MMHP.stan")

fit_mmhp_sep <- immhp_model$sample(data = stan_input_lst,
                                   iter_warmup = 1000,
                                   iter_sampling = 1000,
                                   thin = 4,
                                   chains = 4) 
...
for(pair in 1:nrow(unique_pairs_df)){
  ...
  for(current_sim in 1:1000){
    ## need to modify this, paste the name, etc. 
    ## !!as.name
    lam0_pars <- sim_mmhp_sep %>% select(starts_with("lambda0"))
    lam1_pars <- sim_mmhp_sep %>% select(starts_with("lambda1"))
    alpha_pars <- sim_mmhp_sep %>% select(starts_with("alpha"))
    beta_pars <- sim_mmhp_sep %>% select(starts_with("beta"))
    q1_pars <- sim_mmhp_sep %>% select(starts_with("q1"))
    q2_pars <- sim_mmhp_sep %>% select(starts_with("q2"))
    del_pars <- sim_mmhp_sep %>% select(starts_with("delta"))
    
    param[[current_sim]] <- list(lambda1 = 
                                   as.numeric(lam1_pars[current_sim, pair]),
                                 lambda0 = 
                                   as.numeric(lam0_pars[current_sim, pair]),
                                 alpha = 
                                   as.numeric(alpha_pars[current_sim, pair]),
                                 beta = 
                                   as.numeric(beta_pars[current_sim, pair]),
                                 q1 = 
                                   as.numeric(q1_pars[current_sim, pair]),
                                 q2 = 
                                   as.numeric(q2_pars[current_sim, pair]))
    delta_vec[current_sim] <- as.numeric(del_pars[current_sim, pair])
  }
  ...
  for(current_win in 1:num_winds){
    ...
    row_indicator <- return_df$initiator==current_initiator & 
      return_df$recipient==current_recipient & 
      return_df$observe.id==current_win
    
    if(current_win %in% current_window_vec){
      time_vec <- return_df[row_indicator,"event.times"][[1]]
      observe_period <- unique_observe_win[unique_observe_win$observe.id == 
                                             current_win, "observe.time"]
      state_array_list[[pair]][[current_win]] <- matrix(0,
                                                        nrow = length(time_vec),
                                                        ncol = 1000)
    }
    else {
      time_vec <- NULL
      observe_period <- return_df[return_df$observe.id == current_win,
                                  "observe.time"][1]
      state_array_list[[pair]][[current_win]] <- matrix(0, 
                                                        nrow = 1,
                                                        ncol = 1000)
    ...
    time_segment <- seq(0, observe_period,
                        length.out = no_segments)
    
    #state_array_list[[pair]][[current_win]] <- matrix(0,nrow=length(time_vec),ncol=1000)
    initial_state_list[[pair]][[current_win]] <- matrix(0,
                                                        nrow = 1,
                                                        ncol = 1000)
    termination_state_list[[pair]][[current_win]] <- matrix(0,
                                                            nrow = 1,
                                                            ncol = 1000)
    interpolation_array_list[[pair]][[current_win]] <- matrix(0,
                                                              nrow = no_segments,
                                                              ncol = 1000)
    
    for(current_sim in 1:1000){
      ## latent states at event times
      viterbi_result <- myViterbiWithInitial(events = time_vec,
                                             param = param[[current_sim]],
      ...
      ## interpolation
      latent_inter <- interpolateLatentTrajectory(param[[current_sim]],
                                                  time_vec,
                                                  viterbi_result$zt_v,
                                                  initial.state =
      ...
      interpolation_array_list[[pair]][[current_win]][,current_sim] <- 
        step_fun_est(time_segment)
    }
  }
}
save(state_array_list,
     initial_state_list,
     termination_state_list,
     interpolation_array_list,
     no_segments,
     file = paste(save_data_path,
                  cohort_names[current_cohort],
                "/mmhp_est_zt_",
...
fit_mmhp_sep <- immhp_model$sample(data = stan_train_input_lst,
                                   iter_warmup = 1000,
                                   iter_sampling = 1000,
                                   thin = 4,
                                   chains = 4)
...
mmhp_residual_matrix <- matrix(0,
                               ncol = mice_number,
                               nrow = mice_number)

mmhp_residual_array <- array(0,
                             dim = c(mice_number, mice_number, num_winds))
...
for (i in 1:mice_number) {
  ...
  for (j in 1:mice_number) {
    ...
    if (length(pair) > 0 & (i != j)) {
      par_est <- list(
        lambda0 = mean(lam0_draws %>% pull(pair)),
      ...
      for (cur in c(1:num_winds)) { ## check length > 2
        if (cur %in% current_window_vec) {
          ...
          latent_event <- as.numeric(
            apply(2 - state_array_list[[pair]][[cur_win]],
                                           1, mean) > 0.5)
          ...
          est.intensity.events <- mmhpIntensityAtEvents(
            params = par_est, t = current_event_time,
          ...
          est.intensity <- mmhpIntensityNumeric_win(
            params = par_est,
            t = current_event_time,
            time.vec = time_segment,
            latent.vec = latent_mean
          ...
          all_residual <- all_residual -
            sum(sqrt(est.intensity)) * (time_segment[2] - time_segment[1])
          mmhp_residual_array[i, j, cur] <- -1 * sum(sqrt(est.intensity)) * 
            (time_segment[2] - time_segment[1])
        }

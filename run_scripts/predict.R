...
mmhp_predict_sim <- matrix(list(),ncol=predict_sim,nrow=nrow(to_predice_obs))
...
for(s in 1:predict_sim){
  ...
  for(cur_win in c(1:nrow(to_predice_obs))){
    mmhp_predict_sim[cur_win,s][[1]] <- simulateLatentMMHP(lambda0_matrix = mmhp_par_matrix$lambda0_matrix,

...
for (current_cohort in fit_cohorts) {
  naive_rank_10[[current_cohort]] <- naiveRankHierarchy(
    full_data[[cohort_names[current_cohort]]])
  expert_rank_10[[current_cohort]] <- expertRankHierarchy(
    full_data[[cohort_names[current_cohort]]])
}
...
return_df <- cleanObservationPeriod(current_cohort,
                                    full_data[[cohort_names[current_cohort]]],
                                    clean_data)
to_predice_obs <- unique(
  return_df[(return_df$day >= 16) & (return_df$day <= 21),
...
for (s in 1:predict_sim) {
  ...
  for (cur_win in c(1:nrow(to_predice_obs))) {
    m1_predict_sim[cur_win, s][[1]] <- simulateLatentHP(
      model1_par_matrix$lambda0_matrix,
...
for (s in 1:predict_sim) {
  ...
  for (cur_win in c(1:nrow(to_predice_obs))) {
    m2_predict_sim[cur_win, s][[1]] <- simulateLatentHP(
      model2_par_matrix$lambda0_matrix,
...
for (s in 1:predict_sim) {
  ...
  lam0_draws <- sim_cohort_mmhp %>% 
    select(starts_with("lambda0"))
  lam1_draws <- sim_cohort_mmhp %>% 
    select(starts_with("lambda1"))
  f_draws <- sim_cohort_mmhp %>% 
    select(starts_with("f"))
  lambda_0_est <- as.numeric(lam0_draws[s, ])
  lambda_1_est <- as.numeric(lam1_draws[s, ])
  ...
  model3_par_matrix <- list(
    lambda0_matrix = lam0_par_est,
    # lambda0_matrix_curr,
    lambda1_matrix = lam1_par_est,
  ...
  for (cur_win in c(1:nrow(to_predice_obs))) {
    m3_predict_sim[cur_win, s][[1]] <- simulateLatentMMHP(
      lambda0_matrix = model3_par_matrix$lambda0_matrix,

...
sim_data_path <- "D:/MMHP_Latent_Sims/M1/"  # local location
...
for(sim_num in seq_len(n_sim)) {
  load(paste(sim_data_path, "sim_model1_fit123_", sim_num,
             ".RData", sep=''))
  stan_sims_M1[[sim_num]] <- sim_model1_stan_sim1
  stan_sims_M2[[sim_num]] <- sim_model1_stan_sim2
  stan_sims_M3[[sim_num]] <- sim_model1_stan_sim3
}

# then have all the posterior draws in these 3 list objects

load(paste(sim_data_path, "sim_model1_", 1,
           ".RData", sep = ""))

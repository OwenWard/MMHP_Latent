...
load(paste(data_path,"sim_model3.RData",sep=''))

## fit in the model
sim_model3_stan_fit1 <- rep(list(),n_sim)
sim_model3_stan_fit2 <- rep(list(),n_sim)
sim_model3_stan_fit3 <- rep(list(),n_sim)
...
sim_model3_stan_sim1 <- rep(list(),n_sim)
sim_model3_stan_sim2 <- rep(list(),n_sim)
sim_model3_stan_sim3 <- rep(list(),n_sim)
for(i in 1:n_sim){
  sim_model3_stan_sim1[[i]] <- rstan::extract(sim_model3_stan_fit1[[i]])
  sim_model3_stan_sim2[[i]] <- rstan::extract(sim_model3_stan_fit2[[i]])
  sim_model3_stan_sim3[[i]] <- rstan::extract(sim_model3_stan_fit3[[i]])
}
...
save(object_fn, object_par, object_matrix, sim_model3_data,
     ...
     file = paste(data_path,"sim_model3_fit123.RData",sep=''))
...
for(cur_sim in c(1:n_sim)){
  ...
  interpolate_state_est_lst[[cur_sim]] <- matrix(list(),
                                                 nrow=length(object_par$f_vec_1),
                                                 ncol=length(object_par$f_vec_1))
  mmhp_par_est <- list(lambda0=mean(sim_model3_stan_sim3[[cur_sim]]$lambda0[1001:2000]),
                       lambda1=mean(sim_model3_stan_sim3[[cur_sim]]$lambda1[1001:2000]),
                       eta_1=mean(sim_model3_stan_sim3[[cur_sim]]$eta_1[1001:2000]),
                       eta_2=mean(sim_model3_stan_sim3[[cur_sim]]$eta_2[1001:2000]),
                       eta_3=mean(sim_model3_stan_sim3[[cur_sim]]$eta_3[1001:2000]),
                       beta=mean(sim_model3_stan_sim3[[cur_sim]]$beta[1001:2000]),
                       f=apply(sim_model3_stan_sim3[[cur_sim]]$f[1001:2000,],2,mean))
  clean_sim_data <- cleanSimulationData(raw_data=sim_model3_data[cur_sim][[1]], 
  ...
  for(cur_i in c(1:length(object_par$f_vec_1))){
    for(cur_j in c(1:length(object_par$f_vec_1))[-cur_i]){
      test.mmhp <- sim_model3_data[cur_sim][[1]]$mmhp_matrix[cur_i,cur_j][[1]]
...
save(event_state_est_lst,interpolate_state_est_lst,
     file = paste(data_path,"fit123_state_est.RData",sep=''))

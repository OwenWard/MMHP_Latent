...
n_sim <- 50 # to see how long it takes
cut_off <- 3
...
## Simulate
sim_model2_data <- rep(list())
N_array <- array(0,c(1,5,5))
# for(i in c(1:n_sim)){
sim_model2_data[[1]] <- simulateLatentHP(lambda0_matrix=object_matrix$lambda0_matrix,
...
N_array[1,,] <- clean_sim_data$N_count
# }
...
save(object_fn, object_par, 
     ...
     file = paste(data_path,"sim_model2_",sim_id,".RData",sep=''))



### Fit each model to simulated data ####

load(paste(data_path,"sim_model2_",sim_id,".RData",sep=''))

## fit in the model
sim_model2_stan_fit1 <- rep(list())
sim_model2_stan_fit2 <- rep(list())
sim_model2_stan_fit3 <- rep(list())
...
## Fit in model 1
sim_model2_stan_fit1[[1]] <- sampling(model1, data=data_list, 
                                         iter=1000, chains=4)
print("model2")
## Fit in model 2
sim_model2_stan_fit2[[1]] <- sampling(model2,
                                         data=data_list,
                                         iter=1000, chains=4)
print("model3")
## Fit in model 3
sim_model2_stan_fit3[[1]] <- sampling(model3,
                                         data=data_list,
                                         iter=1000, chains=4)
...
sim_model2_stan_sim1 <- rep(list())
sim_model2_stan_sim2 <- rep(list())
sim_model2_stan_sim3 <- rep(list())
# for(i in 1:n_sim){
sim_model2_stan_sim1 <- rstan::extract(sim_model2_stan_fit1[[1]])
sim_model2_stan_sim2 <- rstan::extract(sim_model2_stan_fit2[[1]])
sim_model2_stan_sim3 <- rstan::extract(sim_model2_stan_fit3[[1]])
# }
...
save(object_fn, object_par, object_matrix, sim_model2_data,
     ...
     file = paste(data_path,"sim_model2_fit123_",sim_id,".RData",sep=''))
...
interpolate_state_est_lst <- matrix(list(),
                                               nrow=length(object_par$f_vec_1),
                                               ncol=length(object_par$f_vec_1))
mmhp_par_est <- list(lambda0=mean(sim_model2_stan_sim3$lambda0[1001:2000]),
                     lambda1=mean(sim_model2_stan_sim3$lambda1[1001:2000]),
                     eta_1=mean(sim_model2_stan_sim3$eta_1[1001:2000]),
                     eta_2=mean(sim_model2_stan_sim3$eta_2[1001:2000]),
                     eta_3=mean(sim_model2_stan_sim3$eta_3[1001:2000]),
                     beta=mean(sim_model2_stan_sim3$beta[1001:2000]),
                     f=apply(sim_model2_stan_sim3$f[1001:2000,],2,mean))
clean_sim_data <- cleanSimulationData(raw_data=sim_model2_data[[1]], 
...
save(event_state_est_lst,interpolate_state_est_lst,
     file = paste(data_path,"fit123_state_est_M2_",sim_id,".RData",sep=''))

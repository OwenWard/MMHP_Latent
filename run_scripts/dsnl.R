...
for (k in c(1:no_days)) {
  ...
  if (k == 1) {
    N_count_day[k, , ] <- cleanDataForTraining(
      all_raw_data = full_data[[cohort_names[current_cohort]]],
      cut_off = 0, N = 12, train_period = 1
    )$N_count
  } else {
    clean_data_test_1 <- cleanDataForTraining(
      all_raw_data = full_data[[cohort_names[current_cohort]]],
      cut_off = 0, N = 12, train_period = k - 1
    )
    clean_data_test_2 <- cleanDataForTraining(
      all_raw_data = full_data[[cohort_names[current_cohort]]],
...
# fit in stan
dsnl_model <- cmdstan_model("lib/dsnl_poisson.stan")

fit_dsnl <- dsnl_model$sample(
  data = list(
    day = no_days,
    Gt = N_count_day,
    c = 1,
...
dsnl_fit <- fit_dsnl$draws()
sim_dsnl <- posterior::as_draws_df(dsnl_fit)
...
N_count_train <- cleanDataForTraining(
  all_raw_data = full_data[[cohort_names[current_cohort]]],
  cut_off = cut_off, N = 12, train_period = 15
)$N_count

dsnl_predict_model <- cmdstan_model("lib/dsnl_predict.stan")

fit_predict_dsnl <- dsnl_predict_model$sample(
  data = list(
    day = 15,
    Gt = N_count_day,
    c = 1,
...
dsnl_pred_fit <- fit_predict_dsnl$draws()
sim_predict_dsnl <- posterior::as_draws_df(dsnl_pred_fit)

...
no_method <- 5
all_cohort_norm_df <- data.frame(norm = rep(NA,10*6*no_method),
...
for(current_cohort in 1:10){
  ...
  load(paste(data_path,cohort_names[current_cohort],
             ...
             ".RData",sep=''))
  clean_data <- cleanData(full_data[[cohort_names[current_cohort]]])
  train_clean_data <- cleanDataForTraining(all_raw_data = full_data[[cohort_names[current_cohort]]],
...
all_cohort_rank_df <- data.frame(spearman = rep(NA,10*6*no_method),
                                 day = rep(NA,10*6*no_method),
                                 method = rep(NA,10*6*no_method),
                                 cohort = rep(NA,10*6*no_method))
...
for(current_cohort in fit_cohorts){
  ...
  predict_day_rank_df <- data.frame(spearman = rep(NA,predict_sim*6*no_method),
                                    day = rep(NA,predict_sim*6*no_method),
                                    method = rep(NA,predict_sim*6*no_method))
  ...
  for(d_test in c(16:21)){
    ...
    for(s in c(1:predict_sim)){
      ...
      unique_pairs_df <- train_return_df %>% group_by(initiator, recipient) %>%
      ...
      for(w in cur_day_windows){ # the window is absolute not change with the day
        ...
        for(i in c(1:mice_number)){
          for(j in c(1:mice_number)[-i]){
            ...
            est_intensity_array_m1[d_test-15,s,i,j] <- max(est_intensity_array_m1[d_test-15,s,i,j],
                                                           uniHawkesIntensity(object=est_par,events=unlist(clean_sim$time_each_pair[i,j]),
        ...
        for(i in c(1:mice_number)){
          for(j in c(1:mice_number)[-i]){
            ...
            names(est_par) <- c("lambda0","alpha","beta")
            est_intensity_array_m2[d_test-15,s,i,j] <- max(est_intensity_array_m2[d_test-15,s,i,j],
                                                           uniHawkesIntensity(object=est_par,events=unlist(clean_sim$time_each_pair[i,j]),
                                                                              current_time=to_predice_obs$observe.time[w]))
          }
        }
        ...
        for(i in c(1:mice_number)){
          for(j in c(1:mice_number)[-i]){
            ...
            est_intensity_array_m3[d_test-15,s,i,j] <- max(est_intensity_array_m3[d_test-15,s,i,j],
                                                           mmhpIntensityAtTime(params=est_par,events=unlist(clean_sim$time_each_pair[i,j]),
                                                                               current_time=to_predice_obs$observe.time[w],
                                                                               latent_z=1,
                                                                               latent_x=0))
          }
        ...
        for(i in c(1:mice_number)){
          for(j in c(1:mice_number)[-i]){
            ...
            names(est_par) <- c("lambda0","lambda1","alpha","beta","q1","q2")
            est_intensity_array_mmhp[d_test-15,s,i,j] <- max(est_intensity_array_mmhp[d_test-15,s,i,j],
                                                             mmhpIntensityAtTime(params=est_par,events=unlist(clean_sim$time_each_pair[i,j]),
                                                                                 current_time=to_predice_obs$observe.time[w],
      ...
      predict_day_rank_df[cur,'spearman'] <- cor.test(real_gl_vec,
                                                      rowSums(est_intensity_array_m1[d_test-15,s,,]), method = "spearman", exact=FALSE)$estimate
      # m2
      predict_day_rank_df[cur+1,'spearman'] <- cor.test(real_gl_vec,
                                                        rowSums(est_intensity_array_m2[d_test-15,s,,]), method = "spearman", exact=FALSE)$estimate
      # m3
      predict_day_rank_df[cur+2,'spearman'] <- cor.test(real_gl_vec,
                                                        rowSums(est_intensity_array_m3[d_test-15,s,,]), method = "spearman", exact=FALSE)$estimate
      # mmhp
      predict_day_rank_df[cur+3,'spearman'] <- cor.test(real_gl_vec,
                                                        rowSums(est_intensity_array_mmhp[d_test-15,s,,]), method = "spearman", exact=FALSE)$estimate
      ...
      predict_day_rank_df[cur+4,'spearman'] <- cor.test(real_gl_vec,
                                                        rowSums(est_intensity_array_dsnl[d_test-15,s,,]), method = "spearman", exact=FALSE)$estimate

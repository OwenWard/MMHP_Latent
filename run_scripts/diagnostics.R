  ...
  for(j in 1:mice_number){
    ...
    if(length(pair)>0&(i!=j)){
      ...
      for(cur in c(1:length(current_window_vec))){ 
        ...
        hawkes.par <- c(model1_par_matrix$lambda0_matrix[i,j],
                        model1_par_matrix$alpha[i,j],
        ...
        all_Lambda <- uniHawkesIntegralIntensity(object = list(
          lambda0 = hawkes.par[1],
          alpha = hawkes.par[2],
          beta = hawkes.par[3]),
          events = current_event_time,
          termination = observe_period)
        all_prresidual <- uniHawkesPearsonResidual(object = list(
          lambda0 = hawkes.par[1],
...
model2_par_est <- list(gamma = apply(gamma_draws, 2, mean),
    ...
    if(length(pair)>0&(i!=j)){
      ...
      for(cur in c(1:length(current_window_vec))){ 
        ...
        observe_period <- return_df[return_df$initiator==i&
                                    ...
                                    "observe.time"]
        #### update this from the stan fit
        hawkes.par <- c(model2_par_matrix$lambda0_matrix[i,j],
                        model2_par_matrix$alpha[i,j],
...
Lambda_cmmhp_matrix <- matrix(rep(list(),
...
alpha_draws <- sim_cohort_mmhp %>% select(starts_with("alpha"))
beta_draws <- sim_cohort_mmhp %>% select(starts_with("beta"))
q1_draws <- sim_cohort_mmhp %>% select(starts_with("q1"))
q2_draws <- sim_cohort_mmhp %>% select(starts_with("q2"))
...
for(i in 1:mice_number){
  ...
  for(j in 1:mice_number){
    pair <- which(unique_pairs_df$initiator==i&unique_pairs_df$recipient==j)
    if(length(pair)>0&(i!=j)){
      par_est <- list(lambda0 = mean(lam0_draws %>% pull(pair)),
      ...
      for(cur in current_window_vec){
        ...
        latent_mean <- apply(interpolation_array_list[[pair]][[cur_win]],
                             1, mean)
        latent_event <- as.numeric(apply(2 -
        ...
        for(m in 1:(length(current_event_time)+1)){
          interevent_idx <- (time_segment>=temp_time_vec[m]) & 
            (time_segment<temp_time_vec[m+1])
          Lambda_vec[m] <- sum(est.intensity[interevent_idx]) * 
...
Lambda_mmhp_matrix <- matrix(rep(list(),
                                 mice_number * mice_number),
                             nrow = mice_number,
                             ncol = mice_number)
...
save(raw_real_c_hawkes_vec,
     pr_real_c_hawkes_vec,
     Lambda_c_hawkes_matrix,
     raw_real_dc_hawkes_vec,
     pr_real_dc_hawkes_vec,
     Lambda_dc_hawkes_matrix,
     raw_real_cmmhp_vec,
     pr_real_cmmhp_vec,
     Lambda_cmmhp_matrix,
     raw_real_mmhp_vec,

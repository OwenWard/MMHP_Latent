...
drawUniMMHPIntensityPaper <- function(object, simulation,
                                      yupper = 10,
                                      add = FALSE,
                                      color = 1,
                                      line.width = 1,
  ...
  if (add == FALSE) {
    plot(0, 0,
      xlim = c(min.x, max.x), ylim = c(min.y, yupper * (1 + y.ratio)),
    ...
    if (annotate.time) {
      ...
      text(seq(0, ceiling(termination), length.out = 5),
        y = -y.ratio * 6, cex = 1.7, srt = 0, xpd = TRUE,
        labels = round(seq(0, ceiling(termination), length.out = 5), 0)
      )
    }
    # events
    points(t[-1], rep(min.y, n - 1),
           cex = event.cex,
           pch = ifelse(simulation$zt[-1] == 1, 16, 1),
           col = "blue")
    points(state_time, 
           rep(lambda0, m),
           cex = 1.3,
  ...
  if ((m == 2 & state[1] == 1 & state_time[2] == tail(t, 1)) | (length(unique(state)) == 1 & state[1] == 1)) {
    drawHawkesIntensityPaper(lambda0,
                             alpha,
                             beta,
                             t,
                             color = color,
...
drawHPIntensityPaper <- function(lambda0,
                                 i,
                                 alpha,
                                 beta,
                                 start,
  ...
  if (n == 0) {
    if (i == 1) {
      ...
      segments(x0 = start, y0 = lambda0,
               y1 = lambda.n(end),
               lty = 2, ## why does this have lty?
               col = color,
               lwd = line.width)
      curve(new.lambda.n,
            from = start,
    ...
    if (i == 1) {
      ...
      curve(new.lambda.n, 
            ...
            lty = line_type)
      segments(x0 = hawkes_time[1],
               y0 = lambda.n(hawkes_time[1]),
               y1 = lambda.n(hawkes_time[1]) + alpha,
               col = color, 
               lwd = line.width, 
               lty = line_type)
    }
    if (n > 1) {
      for (j in 1:(n - 1)) {
        ...
        curve(new.lambda.n,
              ...
              lwd = line.width,
              lty = line_type)
        segments(x0 = hawkes_time[j + 1],
                 y0 = lambda.n(hawkes_time[j + 1]),
                 y1 = lambda.n(hawkes_time[j + 1]) + alpha,
                 col = color,
                 lwd = line.width,
                 lty = line_type)
      }
    ...
    segments(x0 = end,
             y0 = lambda.n(end),
             y1 = lambda0,
             lty = 2, ## didn't change this either
             col = color,
             lwd = line.width)
  }
}

drawHawkesIntensityPaper <- function(lambda0,
                                     alpha,
                                     beta,
                                     events,
                                     color = 1,
  ...
  if (N == 1) { # only one event condition
    ...
    if (horizon > tail(events, 1)) {
      ...
      new.lambda.n <- Vectorize(lambda.n)
      curve(new.lambda.n, from = tail(events, 1), 
            to = horizon,
            add = TRUE,
            col = color,
            lwd = line.width, 
    ...
    for (i in 1:(N - 1)) {
      ...
      curve(new.lambda.n,
            ...
            lwd = line.width,
            lty = line_type)
      segments(x0 = events[i + 1],
               y0 = lambda.n(events[i + 1]),
               y1 = lambda.n(events[i + 1]) + alpha,
               col = color,
               lwd = line.width,
               lty = line_type)
    }
  ...
  for (x.idx in c(1:(length(temp.mmhp$x)))) {
    ...
    if (sum(time.idx) > 0) {
      if (temp.mmhp$z[x.idx] != last.state) {
        ...
        new.x <- c(
          new.x, runif(1, last.event.time, last.event.time + 3),

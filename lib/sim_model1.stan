...
model{
  ...
  for(i in 1:N_til){
    ...
    target += -lambda0*termination - alpha/beta*sum((1-exp(-beta*(termination-segment(event_matrix[i],1,Nm[i])))));
    r[1] = 0; 
    target += log(lambda0+alpha*r[1]);
    if(Nm[i]>1){
      for(n in 2:Nm[i]){
        r[n] = exp(-beta*interevent[n])*(r[n-1]+1); 
        target += log(lambda0+alpha*r[n]);
      }
    }
  }
}

data{
  ...
  int<lower=0,upper=max_Nm> Nm[N_til]; //number of events for each pair => count_matrix
  vector[max_Nm+1] time_matrix[N_til]; // include termination time in the last entry
...
model{
  ...
  for(i in 1:N_til){ //for each pair
    ...
    //for(j in 1:no_observations){ // for each observation period
      ...
      if(Nm[i]==0){ // there is no event occured in this period
        ...
        for(n in 1:(Nm[i]+1)){ //changed this
        ...
        forward_termination[1] = log_sum_exp(forward[Nm[i]] + probs_1[Nm[i]] - int_1[Nm[i]]);
        forward_termination[2] = log_sum_exp(forward[Nm[i]] + probs_2[Nm[i]] - int_2[Nm[i]]);

...
matrixPlotParameter <- function(x, ...) {
  ...
  ColorRamp <- colorRampPalette(rev(brewer.pal(8, "RdBu")))(length(xLabels) *
                                                              length(yLabels))
  ...
  axis(BELOW <- 1, at = 1:length(xLabels),
       labels = xLabels,
       cex.axis = 1.1, tck = -0.015, gap.axis = 0.2)
  axis(LEFT <- 2,
...
my.image.plot <- function(..., add = FALSE, xlab = NULL, ylab = NULL,
  ...
  if (!horizontal) {
    ...
    axis(4,
      at = c(0, 0.8, 1.6, 2.4, 3.2), labels = c(0, 0.8, 1.6, 2.4, 3.2), las = HORIZONTAL <- 1,
    ...
    axis(4,
      at = c(0, 0.6, 1.2), labels = c(0, 0.6, 1.2), las = HORIZONTAL <- 1,
  ...
  if (!is.null(legend.lab)) {
    legend.args <- list(
      text = legend.lab, side = ifelse(horizontal,
        1, 4
      ),
...
myMultiMatrixPlot <- function(X, xLabels, yLabels, min, max, colorPalette, no_matrix, n_row,
  ...
  if (no_matrix == 2) {
    ...
    layout(cbind(
      matrix(data = c(1:no_matrix), nrow = n_row, ncol = no_matrix / n_row, byrow = TRUE),
  ...
  for (i in 1:no_matrix) {
    ...
    image(1:length(xLabels_temp), 1:length(yLabels_temp), t(x),
      col = ColorRamp, xlab = "",
    ...
    if (is.null(title.line)) {
      ...
      axis(BELOW <- 1, at = 1:length(xLabels_temp), 
           labels = xLabels_temp,
           cex.axis = axis_cex, gap.axis = 0.2)
      axis(LEFT <- 2, at = 1:length(yLabels_temp),
           labels = yLabels_temp,
           las = HORIZONTAL <- 1,
           cex.axis = axis_cex,
           gap.axis = 0.2)
      ...
      mgp.axis(2,
        mgp = c(0, title.line$left, 0), at = 1:length(yLabels_temp), labels = yLabels_temp,

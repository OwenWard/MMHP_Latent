...
```{r simulate_20_nodes, eval=FALSE, include=FALSE}
num_nodes <- 20
cut_off <- 3
obs_time <- 200
...
```{r plot_pair, eval=FALSE, include=FALSE}
start <- 20
end <- 10
...
```{r residuals-immhp-sim, eval=FALSE, include=FALSE}
...
```{r repeat-all-cohorts, eval=FALSE, include=FALSE}
...
all_ks_p %>% ggplot(aes(Method, ks)) +
  geom_boxplot(aes(fill = Method)) +
  scale_fill_manual(values = cols_plot) +
  labs(y = "P Value", title = "All Simulations")
...
all_ks_p %>% ggplot(aes(Method, ks)) +
  geom_boxplot(aes(fill = Method)) +
  scale_fill_manual(values = cols_plot) +
  labs(y = "P Value", title = "single simulation")


```


```{r take-mean-ks/pr, include=FALSE}
m1_ks <- tibble(Method = "C-HP",

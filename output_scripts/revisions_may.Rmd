...
#### more legend
legend(legend.x[1], legend.y[1] + 1, c("True"),
       col = c("black"),
...
### used legend
legend(legend.x[2], legend.y[2] + 1, c("True"),
       col = c("black"),
       y.intersp = 0.88, x.intersp = 0.15, bty = "n",
       lty = c(1), lwd = c(5), cex = legend.cex, seg.len = 1
)
...
points(legend.x[2] - 73, legend.y[2] - 0.5, 
       pch = 1, cex = 2, col = "blue")
points(legend.x[2] - 69, legend.y[2] - 0.5,
...
drawUniMMHPIntensityPaper(true.object,
                          ...
                          color = add.alpha("black", alpha = line.alpha[1]),
                          line.width = line.wdth[1],
                          y.ratio = -0.05, min.y = -0.5, min.x = 0, max.x = my.xlim,
...
legend(legend.x[3], legend.y[3] + 1, c("True"),
       col = c("black"),
       y.intersp = 0.88, x.intersp = 0.15, bty = "n",
       lty = c(1), lwd = c(5), cex = legend.cex, seg.len = 1
)
...
legend(legend.x[3] - 80, legend.y[3] + 2,
...
m1_ks <- tibble(Method = "C-HP",
       ks = as.vector(KS_model1_array))

m2_ks <- tibble(Method = "C-DCHP",
       ks = as.vector(KS_model2_array))

m3_ks <- tibble(Method = "C-MMHP",
       ks = as.vector(KS_model3_array))

true_ks <- tibble(Method = "True",
                  ks = as.vector(KS_true_array))
...
m1_pr <- tibble(Method = "C-HP",
       pr = as.vector(PR_model1_array))

m2_pr <- tibble(Method = "C-DCHP",
       pr = as.vector(PR_model2_array))

m3_pr <- tibble(Method = "C-MMHP",
       pr = as.vector(PR_model3_array))

true_pr <- tibble(Method = "True",
                  pr = as.vector(PR_true_array))

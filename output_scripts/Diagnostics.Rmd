...
drawRealIntensityPaper<-function(t,object=NULL,simulation=NULL,yupper=10,add=FALSE,color=1,line.width=1,
                                 title_name="",title.cex=9, y.ratio=0, min.y=0, min.x=0, max.x=NULL, 
                                 annotate.time=FALSE, event.cex=1, box.type="l", box.col="gray40", title.line=1){
...
drawHawkesIntensityPaper<-function(lambda0,alpha,beta,events,color=1,line.width=2, termination=NULL){
...
```{r, fig.width=10}
#par(mfrow=c(2,5), oma=c(1,1,1,1), mar=c(2,3,2,1), mgp=c(0.8,0,0))
#layout(matrix(1:10,nrow=2,byrow=TRUE),widths=rep(1,5),height=c(3,3))
anno.cex <- 1.3
title.cex <- 2.2
title.line <- 0.5
y.upper <- 20
## pp
lambda.pp <- 1/mean(diff(c(0,current_event_time,observe_period)))
drawRealIntensityPaper(current_event_time,
                       yupper=y.upper, color="black",line.width=2,title.line=title.line,
                       title_name="Poisson", title.cex=title.cex, max.x=observe_period, box.col="black")
abline(h=lambda.pp,lwd=2)
mtext(side=1, text="Time", line=1.6, cex=anno.cex)
mtext(side=2, text="Intensity", line=1.2, cex=anno.cex)

## Hawkes
hawkes.par<-optim(par=rep(1,3), fn=uniHawkesNegLogLik, method="L-BFGS-B", 
                  t=current_event_time, termination = observe_period, lower=c(0,1,0))

drawRealIntensityPaper(current_event_time,
                       yupper=y.upper, color="black",line.width=2,title.line=title.line,
                       title_name="Hawkes", title.cex=title.cex, max.x=observe_period, box.col="black")
drawHawkesIntensityPaper(lambda0=hawkes.par$par[1],
...
```{r load_mmhp_data, cache=TRUE}
data_path <- "C:/Users/owenw/Google Drive/Tian/Current_Projects/MMHP_Latent/output_june_26/output//"
...
mmhp_par <- list(lambda1=mean(sim_cohort_mmhp$lambda1),
                 lambda0= (mean(sim_cohort_mmhp$gamma[,current_i])+
                             mean(sim_cohort_mmhp$zeta[,current_j])),
                 alpha=mean(sim_cohort_mmhp$alpha[,pair]),
                 beta=mean(sim_cohort_mmhp$beta),
                 q1=mean(sim_cohort_mmhp$q1[,pair]),
                 q2=mean(sim_cohort_mmhp$q2[,pair]))

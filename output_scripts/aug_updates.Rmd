...
```{r further checking permutation}
...
```{r}
real_cleaned <- cleanData(full_data[[cohort_names[current_cohort]]])

permuted <- permute_raw(current_cohort)
permuted_cleaned <- cleanData(permuted)

permuted_cleaned$N_count - real_cleaned$N_count
# these look different which is good

return_df <- cleanObservationPeriod(current_cohort,
                                    full_data[[cohort_names[current_cohort]]],
                                    real_cleaned)

per_return_df <- cleanObservationPeriod(current_cohort,permuted,permuted_cleaned)

return_df$initiator - per_return_df$initiator
# so this is the issue then possibly
...
mice_covars %>% ggplot(aes(true_rank,perm_rank)) + geom_point()

mice_covars %>% ggplot(aes(true_gam,perm_gam)) + geom_point()

mice_covars %>% ggplot(aes(true_zeta,perm_zeta)) + geom_point()

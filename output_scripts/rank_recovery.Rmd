...
Similarly, we can compute the Spearman rank correlation between each of these 
models and the truth.

```{r some code to summarise these, eval = FALSE}
### compute spearman rank
truth <- output_rank$truth
output_rank %>% 
  select(m1:isi) %>%
...
output_rank2 <- output_rank
output_rank2$sim <- 2
output_rank2


bind_rows(output_rank, output_rank2) %>%
  group_by(sim) %>%
  select(m1:isi) %>%
  map( ~cor.test(truth, .x,
                 method = "spearman")$estimate  ) %>%
...
sims_list[[1]] <- output_rank
sims_list[[2]] <- output_rank2
...
map_dfr(sims_list, spear) # in a nice long format
```
...
compute_isi(count_data_dc, curr_ranking = c(5,3,1,2,4))
...
agg_rank_train <- order(agg_train_mean_rank)
```

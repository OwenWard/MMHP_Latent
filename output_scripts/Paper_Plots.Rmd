...
# # then same plot as above with diff
# col.vec <- cRamp(as.vector(diff))
# ## need to do some sort of transformation between these here
# vir_cols <- viridis::viridis(length((as.vector(diff))))
# #order(as.vector(ks.stat.matrix),method = "")
# 
# values <- as.vector(diff)
# sorted_vals <- sort(diff)
# #match(values,sorted_vals)
# col.vec.vir <- vir_cols[match(values,sorted_vals)]
# 
# ####
...
# par(mar = c(0,2,0,3),mgp=c(0,0.8,0))
# 
# col_obj <- list(x.vec = seq(min(as.vector(diff)),max(as.vector(diff)),
#                             length.out = 100),
#                 col.vec = viridis(100))
# 
# # temp <- cLegend(as.vector(ks_diff))
...
#       side = 2, line = 0.5, outer = TRUE, cex = 1.8,
#       at =1/12*c(12:1)-1/20, las = 2)
# mtext(text = "M2-M1 normalised by num events", side = 3, line = 1.4, outer = TRUE,
#       cex = 2.2,
#       at=0.45, font=2)

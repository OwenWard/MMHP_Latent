...
```{r prepare data for 4b,cache=TRUE, eval=FALSE}
...
sim_data_path <- here("output", "revisions", "sim_m3", "june_18", "/")

load(paste0(
  sim_data_path, "/", "sim_model3_",
  plot.s,
  ".RData"
))
...
```{r fig-4b-recover-latent-states,fig.width=8,fig.height=4, eval=FALSE}
mmhp_lty <- 1
...
load("../output/revisions/weighted_rank_data.RData")
...
```{r fig-8a-frob-norm-cohort5, fig.width=8,fig.height=4,eval=TRUE}
load("../output/revisions/plot_N_predict_revision.RData")
...
```{r fig-7a-pred-all-cohorts-box-mae}
### updated to revision
load("../output/revisions/plot_N_predict_revision.RData")
...
for (t in 1:length(task.types)) {
  ...
  cur_real_rank <- real.gl$history[, day_end_idx[t], 1]
  for (s in c(1:1000)) {
    temp_result <- cor.test(-sim_dsnl_active$x[s, t, ], cur_real_rank,
                            method = "spearman")
    temp_vec[s] <- temp_result$estimate
  }

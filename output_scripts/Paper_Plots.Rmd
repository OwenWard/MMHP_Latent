...
```{r fig-2a-contour-plot, eval=revision_plots}
current_cohort <- 5
...
```{r fig-2b-diag-plot,echo=FALSE,warning=FALSE, eval=revision_plots}
# current_cohort <- 5
current_cohort <- 5
...
```{r fig-3a-baseline-rate, eval=revision_plots}
current_cohort <- 5
...
```{r fig-3b-residual-structure, include=TRUE, eval=revision_plots}
...
for (i in seq_along(sim_files)) {
  single_fit <- readRDS(here(
    "output", "revisions", "lapl_check", "sim_rank",
    sim_files[i]
  ))
...
```{r fig-7a-pred-all-cohorts-box-mae, fig.width=8,fig.height=4,eval=TRUE}
### updated to revision
load("../output/revisions/lapl_check/plot_N_predict_revision.RData")

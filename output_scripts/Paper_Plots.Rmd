...
matrixPlotParameter(latent_social_matrix[rev(expert_rank_10[[current_cohort]]),
...
```{r fig-4a}
sim_data_path <- "../output/sims_long/"
...
## model 1
#########
model1_par_est <- list(lambda0=mean(sim_model3_stan_sim1[[plot.s]]$lambda0[1001:2000]),
...
lambda.m1<-uniHawkesIntensityNumeric(object=list(lambda0=model1_par_est$lambda0,
...
## model2
#############
model2_par_est <- list(gamma=apply(sim_model3_stan_sim2[[plot.s]]$gamma[1001:2000,],2,mean),
                       zeta=apply(sim_model3_stan_sim2[[plot.s]]$zeta[1001:2000,],2,mean),
...
lambda.m2<-uniHawkesIntensityNumeric(object=list(lambda0=model2_par_est$gamma[plot.i]+
...
state.est.latent.mmhp.new <- fixInterpolateState(state.est.latent.mmhp, termination=200)

# where is fixInterpolateState?...
# this is in one of the files at the top of latent_simulation.Rmd
# drawIntensity.R
...
lambda.true <- mmhpTrueIntensityNumeric(params=object_true, t=temp.t,
...
my.xlim <- 200#130
my.long.x <- 205#134.5
legend.x <- c(135,135,135)
legend.y <- #c(34,32,32)
legend.y <- c(62,37,18)#c(22,22,22)
legend.cex <- 1.5 #3
...
# axis(2,at=c(-10,y.ub[1]),col="gray35",line=0,tick=T,labels=rep("",2),lwd=0.7,lwd.ticks=0)
# axis(3,at=c(-5,my.long.x),col="gray35",line=0,tick=T,labels=rep("",2),lwd=0.7,lwd.ticks=0)
# axis(4,at=c(-10,y.ub[1]),col="gray35",line=-0.5,tick=T,labels=rep("",2),lwd=0.7,lwd.ticks=0)

## add legend
# changed 34 to 23, 25 to 18, -20 to -10
legend(20,104,c("State 1/0 events","State change point"),
       col = c(NA,"red"),
...
# legend(93,24,c("Overestimation","Underestimation"),
#        col = c(positive.col,negative.col),
#        y.intersp=0.88,x.intersp=0.5,bty = "n",
#        lty = c(1,1), lwd = c(20,20), cex=legend.cex,seg.len=0.8)
...
polygon(c(0,lambda.true$time.vec,200),
...
## draw box
# change -0.7 to -0.5 and -0.5 to -0.25
axis(1,at=c(-2,my.long.x),col="gray35",
     line=0,tick=T,labels=rep("",2),lwd=0.7,lwd.ticks=0)
# axis(2,at=c(-2,y.ub[1]),col="gray35",
#      line=0,tick=T,labels=rep("",2),lwd=0.7,lwd.ticks=0)
# axis(4,at=c(-2,y.ub[1]),col="gray35",
#      line=-0.5,tick=T,labels=rep("",2),lwd=0.7,lwd.ticks=0)
...
drawHawkesIntensityPaper(lambda0=model2_par_est$gamma[plot.i]+
                         ...
                         beta=model2_par_est$beta,
                         events=test.mmhp$tau,
                         color=add.alpha(model_colors[2], alpha=line.alpha[2]),line.width=line.wdth[2])
...
axis(1,at=c(-2,my.long.x),col="gray35",
     line=0.5,tick=T,labels=rep("",2),lwd=0.5,lwd.ticks=0,lty=3)
# axis(2,at=c(-5,y.ub[2]),col="gray35",line=0,tick=T,labels=rep("",2),lwd=0.7,lwd.ticks=0)
# axis(3,at=c(-5,my.long.x),col="gray35",line=0,tick=T,labels=rep("",2),lwd=0.7,lwd.ticks=0)
# axis(4,at=c(-5,y.ub[2]),col="gray35",line=-0.5,tick=T,labels=rep("",2),lwd=0.7,lwd.ticks=0)
...
delta.negative[delta.negative>0] <- 0
#### hiding these for now ####
polygon(c(0,lambda.true$time.vec,200), c(0,delta.negative,0),col=negative.col,border=NA)
polygon(c(0,lambda.true$time.vec,200), c(0,delta.positive,0),col=positive.col,border=NA)
# segments(0,0,my.xlim,0,col="gray45")
## draw box
axis(1,at=c(-2,my.long.x),col="gray35",
     line=0,tick=T,labels=rep("",2),lwd=0.7,lwd.ticks=0)
# axis(2,at=c(-20,y.ub[2]),col="gray35",line=0,tick=T,labels=rep("",2),lwd=0.7,lwd.ticks=0)
# axis(4,at=c(-20,y.ub[2]),col="gray35",line=-0.5,tick=T,labels=rep("",2),lwd=0.7,lwd.ticks=0)

# model3
########
par(mar = c(0,0.3,1,0.1))
drawUniMMHPIntensityPaper(true.object,simulation=fixStateTransition(test.mmhp),
                          yupper=y.ub[3], color=add.alpha("black", alpha=line.alpha[1]),line.width=line.wdth[1],
                          y.ratio=-0.05, min.y=-2, min.x=0, max.x=my.xlim, 
                          box.type="n")
drawUniMMHPIntensityPaper(object_hat,simulation=list(
...
axis(1,at=c(-2,my.long.x),col="gray35",
     line=0.5,tick=T,labels=rep("",2),lwd=0.5,lwd.ticks=0,lty=3)
# axis(2,at=c(-20,y.ub[3]),col="gray35",line=0,tick=T,labels=rep("",2),lwd=0.7,lwd.ticks=0)
# axis(3,at=c(-5,my.long.x),col="gray35",line=0,tick=T,labels=rep("",2),lwd=0.7,lwd.ticks=0)
# axis(4,at=c(-20,y.ub[3]),col="gray35",line=-0.5,tick=T,labels=rep("",2),lwd=0.7,lwd.ticks=0)

legend(55,32,c("Overestimation","Underestimation"),
       col = c(positive.col,negative.col),
       y.intersp=0.88,x.intersp=0.5,bty = "n",
...
axis(1,at=c(-2,my.long.x),col="gray35",
     line=0,tick=T,labels=rep("",2),lwd=0.7,lwd.ticks=0)
# axis(2,at=c(-20,y.ub[1]),col="gray35",
#      line=0,tick=T,labels=rep("",2),lwd=0.7,lwd.ticks=0)
# axis(4,at=c(-20,y.ub[1]),col="gray35",line=-0.5,tick=T,labels=rep("",2),lwd=0.7,lwd.ticks=0)
...
```{r fig-rank-slides, fig.height=3}
...
```{r fig_pr_slides, fig.height=3, fig.width=7}
current_cohort <- 5
...
```{r fig-slides-predict}
load("../output/plot_N_predict_new.RData")
temp_plot_df <- all_cohort_norm_df %>% 
  group_by(day, method) %>% 
  summarise(avg=mean(norm), n= n(), sd=sd(norm),
            se = sd/sqrt(n))
...
```{r construct data for plot, results='hide',message=FALSE,cache=FALSE}

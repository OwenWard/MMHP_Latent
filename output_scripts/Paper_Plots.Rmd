...
knitr::opts_chunk$set(
  echo = TRUE, fig.path = "paper_figure/revision/",
  dev = "png", fig.height = 5, fig.width = 7,
...
```{r creat colour df, eval=revision_plots, include=FALSE}
# col_df <- tibble(
#   method = c(
#     "I&SI", "AggRank", "DSNL", "Glicko", "I-MMHP",
#     "C-HP", "C-DCHP", "True", "C-MMHP"
...
```{r fig-2a-contour-plot, eval=revision_plots, fig.width=6, fig.height=6}
current_cohort <- 5
...
z.func <- function(x, y) {
  return(x * y * eta1_mean *
           exp(-eta2_mean * abs(x - y)) / (1 + exp(-eta3_mean * (y - x))))
...
```{r fig-2b-diag-plot-c5,echo=FALSE,warning=FALSE, eval=revision_plots, fig.width=6, fig.height=6}
# current_cohort <- 5
current_cohort <- 5
...
ks_pval_chp <- matrix(0, nrow = mice_number, ncol = mice_number)
  ...
  for (l in c(1:mice_number)[-k]) {
    ...
    if (length(pair) > 0) {
      ...
      for (cur in c(1:length(current_window_vec))) { ## check length > 2
        ...
        Lambda_vec <- uniHawkesCompensator(
          object = param_mean,
          events = c(
            current_event_time,
            current_obs_time
      ...
      if (length(all_Lambda) > 0) {
        ks.stat.matrix[i, j] <- ks.test(all_Lambda, "pexp",
          rate = 1 / mean(all_Lambda)
        )$statistic
        ks_pval_chp[i, j] <- ks.test(all_Lambda, "pexp",
          rate = 1 / mean(all_Lambda)
        )$p.value
...
####
par(
  oma = c(6, 4, 4, 1), mar = rep(0, 4),
  mgp = c(0, .2, 0)
)
layout(cbind(matrix(
  data = c(1:(12 * 12)), nrow = 12, ncol = 12,
  byrow = TRUE
), rep(12 * 12 + 1, 12)),
...
for (i in expert_rank_10[[current_cohort]]) {
  for (j in expert_rank_10[[current_cohort]]) {
    if (ks.stat.matrix[i, j] == 0) {
      plot(0, 0,
        xlim = c(0, 5), ylim = c(0, 5), type = "n", xlab = "", ylab = "",
      ...
      plot(ecdf(Lambda.test),
        add = TRUE, do.points = FALSE,
...
col_obj <- list(
  x.vec = seq(0, max(as.vector(ks.stat.matrix)), length.out = 100),
  col.vec = viridis(100)
)
...
axis(4,
  at = c(0, 0.3, 0.6), labels = c(0, 0.3, 0.6), las = HORIZONTAL <- 1,
  cex.axis = 2.4
)

mtext(
  text = expert_rank_10[[current_cohort]],
...
mtext(
  text = "(b)", side = 3, line = 1.4, outer = TRUE,
  cex = 2.2,
  at = 0.45, font = 2
)
# dev.off()
ks_chp <- ks.stat.matrix
current_cohort <- 6
```
...
```{r fig-3a-baseline-rate, fig.width=5, fig.height=5}
current_cohort <- 5
## this looks better
# current_cohort <- 6 # 3 is maybe ok, 6/7 is interesting
...
zeta_mean <- apply(zeta_draws, 2, mean)
...
## Test figure (not used)

KS Plot for C-DCHP

```{r ks-c-dchp, eval=FALSE, warning=FALSE}
ks.stat.matrix <- matrix(0, ncol = mice_number, nrow = mice_number)
ks_pval_dchp <- matrix(0, ncol = mice_number, nrow = mice_number)
n_matrix <- matrix(0, ncol = mice_number, nrow = mice_number)
Lambda_matrix <- matrix(list(), nrow = mice_number, ncol = mice_number)
...
for (k in 1:mice_number) {
  for (l in c(1:mice_number)[-k]) {
    ...
    if (length(pair) > 0) {
      param_mean <- list(
        lambda0 = gamma_mean[i] + zeta_mean[j],
        ## need to modify this
        # lambda0 <- mean(sim_cohort_dchp$gamma)
        alpha = model1_fn$alpha.fun(
          f_est[i],
          f_est[j],
          eta1_mean,
      ...
      if (length(all_Lambda) > 0) {
        ...
        ks_pval_dchp[i, j] <- ks.test(all_Lambda, "pexp",
          rate = 1 / mean(all_Lambda)
        )$p.value
        Lambda_matrix[i, j][[1]] <- all_Lambda
      }
...
mtext(
  text = "C-DCHP", side = 3, line = 1.4, outer = TRUE,
  cex = 2.2,
  at = 0.45, font = 2
)
# dev.off()

ks_dchp <- ks.stat.matrix

```


Plot the negative log of the p values here for all cohorts.

```{r above across all_cohorts, eval=FALSE}

ks_chp_array <- array(0, dim = c(10, mice_number, mice_number))
ks_dchp_array <- array(0, dim = c(10, mice_number, mice_number))
    ...
    for (l in c(1:mice_number)[-k]) {
      ...
      if (length(pair) > 0) {
        ...
        if (length(all_Lambda) > 0) {
          ...
          ks_chp_array[current_cohort, i, j] <- ks.test(all_Lambda,
                                                        "pexp",
            rate = 1 / mean(all_Lambda)
          )$p.value
          Lambda_matrix[i, j][[1]] <- all_Lambda
    ...
    for (l in c(1:mice_number)[-k]) {
      ...
      if (length(pair) > 0) {
        ...
        if (length(all_Lambda) > 0) {
          ...
          ks_dchp_array[current_cohort, i, j] <- ks.test(all_Lambda, "pexp",
            rate = 1 / mean(all_Lambda)
          )$p.value
          Lambda_matrix[i, j][[1]] <- all_Lambda
        }
...
```{r plot-all-cohorts, eval=FALSE}
M1_ks_p <- tibble(ks_p = as.vector(ks_chp_array), method = "C-HP")

M2_ks_p <- tibble(ks_p = as.vector(ks_dchp_array), 
                  method = "C-DCHP")

ks_p <- bind_rows(M1_ks_p, M2_ks_p) %>% 
  left_join(col_df, by = "method")

cols_plot <- col_df %>%
  filter(method %in% c("C-HP", "C-DCHP")) %>% 
  pull(cols)

exp_fcn <- function(x) exp(-x)

ks_p$method <- factor(ks_p$method, levels = c("C-HP", "C-DCHP"))

ks_p %>% 
  filter(ks_p != 0) %>% 
  mutate(log_p = -log(ks_p)) %>% 
  ggplot(aes(x = log_p)) + 
...
Further diagnostic plots for C-HP vs C-DCHP


```{r plot-pvalues-m1-m2-here, eval=FALSE}
bind_cols(M1_ks_p, M2_ks_p) %>% 
  rename(chp_p = ks_p...1, cdchp_p = ks_p...3) %>% 
  filter(chp_p != 0) %>% 
  ggplot(aes(chp_p, cdchp_p)) + geom_point() +
  labs(x = "C-HP p Value", y = "C-DCHP p Value",
       title = "p Value for C-HP and C-DCHP") +
  theme_minimal()
...
## Figure 3b: Residual Structure

```{r fig-3b-residual-structure, include=TRUE, fig.width=10, fig.height=5, eval=revision_plots}
### this looks better
### now to add Pearson residual matrices here for C-HP and C-DCHP from fig 7
current_cohort <- 5
...
myMultiMatrixPlot(
  X = matrix_lst_plot, no_matrix = 2, n_row = 1,
  xLabels = expert_rank_10[[current_cohort]],
  yLabels = rev(expert_rank_10[[current_cohort]]),
  min = -90, max = 90, axis_cex = 1.6, title_cex = 1.8,
  colorPalette = "RdBu", if.emp = FALSE,
  # legend.mar=c(0.5,.5,0.5,0.5),
  title_lst = list("C-HP", "C-DCHP", "C-MMHP"),
  # title_lst = list("I-MMHP"),
  col_axis = c(-90, -45, 0, 45, 90), fake_matrix = TRUE,
  matrix.mar = c(2.5, 2.5, 2.5, 1)
)
mtext(text="(b)",side=3,line=0.5,outer=TRUE,cex=2.5,font=2)

```


## Additional Cohort for Response

Add this below figure to the response to show how different cohorts 
result in different response.

```{r fig3-cohort-response, fig.width=5, fig.height=5}
current_cohort <- 6
### this looks good
...
Reformatting this to boxplots for each of the 3 models, with the true
ranking values included for each.

Will need to update this with full final simulations but fully set up now.

```{r fig-4a, eval = fig4, message=FALSE, warning=FALSE}
sim_data_path <- here("output", "revisions", "aug_run", "sims", "/")
...
## change this to just list all files with a pattern
sims <- list.files(sim_data_path, pattern = "sim_model3_fit123")
...
true_ranks <- tibble(node = rep(1:num_nodes,3),
                     true_rank = rep(f_vec,3),
                     model = c(rep("C-HP", 10), rep("C-DCHP", 10),
                               rep("C-MMHP", 10)))
...
## this is just the median from each simulation, not the posterior draws
```

## Figure 4b: Recover Latent States


Need to modify this after finally correctly formulating the 
simulations.

```{r prepare data for 4b,cache=TRUE, eval=fig4}
...
```{r fig-4b-recover-latent-states,fig.width=8,fig.height=4, eval=fig4}
mmhp_lty <- 1
...
legend.y <- # c(34,32,32)
legend.y <- c(3.5, 3.5, 3.5) # c(15,12,15)#c(62,37,18)#c(22,22,22)
legend.cex <- 1.5 # 3
line.alpha <- c(1, 0.5)
line.wdth <- c(1.5, 2)
...
points(legend.x[2] - 33, legend.y[2] + 0.5, 
       pch = 1, cex = 2, col = "blue")
points(legend.x[2] - 31, legend.y[2] + 0.5,
...
Confirm parameter recovery for some of the simulation settings, as 
a potential figure 5a also.

```{r param-recov}
## param recovery for some lambda0, lambda1, alpha, beta,
## eta1, eta2, eta3
model1_fn <- list(alpha.fun = function(x, y, eta1, eta2, eta3){
...
```{r process-sims}
### load in sims, take mean of each parameter from each simulation 
### setting, add to tibble?
sim_data_path <- here("output", "revisions", "lapl_check", "sim_lapl", "/")

sim_files <- list.files(here("output",
                             "revisions",
                             "lapl_check",
                             "sim_lapl",
                             "/"), 
                        pattern = "sim_model3_fit123_")
...
for(i in seq_along(sim_files)) {
  ...
  #then get the ests
  m1_est <- sim_model3_stan_sim1 %>%
    select(beta, eta_1, eta_2, eta_3) %>% 
    summarise(across(.cols = everything(), median)) %>%
    pivot_longer(cols = everything()) %>% 
    mutate(sim_id = i, model = "C-HP")
  
  m2_est <- sim_model3_stan_sim2 %>%
    select(beta, eta_1, eta_2, eta_3,
           starts_with("gamma"),
           starts_with("zeta")) %>% 
    summarise(across(.cols = everything(), median)) %>%
    pivot_longer(cols = everything()) %>% 
    mutate(sim_id = i, model = "C-DCHP")
  
  m3_est <- sim_model3_stan_sim3 %>%
    select(beta, eta_1, eta_2, eta_3, 
           starts_with("gamma"),
           starts_with("zeta")) %>% 
    summarise(across(.cols = everything(), median)) %>%
    # rename(w_lambda = r_lambda1) %>% 
    pivot_longer(cols = everything()) %>% 
    mutate(sim_id = i, model = "C-MMHP")
  par_ests <- par_ests %>% bind_rows(m1_est, m2_est, m3_est)
}

par_ests

true_pars <- tibble(name = c("beta", "eta_1", "eta_2", "eta_3",
                             paste0("gamma[", 1:10, "]"),
                             paste0("zeta[", 1:10, "]")),
                    value = c(1.5, 3, 2, 2.5, seq(from = 0.01, to = 0.2,
                                   length.out = num_nodes),
                              rep(0.1, num_nodes)))

par_ests %>% 
  filter(name %in% c("beta", "eta_1", "eta_2", "eta_3")) %>% 
  ggplot(aes(model, value)) + geom_boxplot() +
  geom_hline(data = true_pars %>% 
               filter(name %in% c("beta", "eta_1", "eta_2", "eta_3")),
...
For completeness, we also wish to repeat the
above two plots using data generated 
from M1 and M2.
...
par(
  mfrow = c(3, length(object_par$f_vec_1)),
  mar = c(0, 0, 1, 0),
  oma = c(6.5, 11, 3.5, 1)
)
...
for (m in c(1:3)) {
  ...
  ## bottom
  mtext(
    text = expression(f[1], f[2], f[3], f[4], f[5]), side = 1, line = 2.4, outer = TRUE,
    at = c(1:length(object_par$f_vec_1)) / length(object_par$f_vec_1) - 0.1,
    cex = 2.4
  )
  mtext(
    text = "Latent rank variables", side = 1, line = 5.3, outer = TRUE, cex = 2.7,
    at = 0.5
  )
  ## top
  mtext(
    text = "Simulated from M1", side = 3, line = 0, outer = TRUE, cex = 3,
    at = 0.5, font = 2
  )
  ## left
  mtext(
    text = rev(c("C-HP  ", "C-DCHP", "C-MMHP")), side = 2, line = 0.5, outer = TRUE,
    at = c(0:2) / 3 + 0.15,
    cex = 2, las = 2
  )
}
...
for (m in c(1:3)) {
  ...
  ## top
  mtext(
    text = "Simulated from M2", side = 3, line = 0, outer = TRUE, cex = 3,
    at = 0.5, font = 2
  )
  ## left
  mtext(
...
clean_sim_data_m1 <- cleanSimulationData(
  raw_data = sim_model1_data[[1]],
  cut_off = cut_off,
  N = length(object_par$f_vec_1)
)
...
model2_par_est <- list(
  gamma = apply(
    sim_model1_stan_sim2_total[[plot.s]]$gamma,
    2, mean
  ),
  zeta = apply(
    sim_model1_stan_sim2_total[[plot.s]]$zeta,
    2, mean
  ),
  ...
  f = apply(
    sim_model1_stan_sim2_total[[plot.s]]$f,
    2, mean
  )
)
...
lambda.true <- uniHawkesIntensityNumeric(
  object = object_true,
  events = temp.t,
  time.vec = time.segment
)
...
par(
  mar = c(0, 0.3, 0, 0.1), oma = c(3, 3.5, 4, 0),
  tcl = 0.2, mgp = c(0.5, 0, 0), xpd = TRUE
)
...
mod_drawHawkesIntensityPaper(
  lambda0 = true.object$lambda0,
...
plot(0, 0,
  xlim = c(0, my.xlim), ylim = c(-4, 10), type = "n",
...
plot(lambda.true$time.vec, delta.lambda,
  xlim = c(0, my.xlim), ylim = c(-3, 10), type = "n",
  bty = "n", xlab = "", ylab = "", xaxt = "n", yaxt = "n", axes = FALSE
)
delta.lambda <- lambda.m2$lambda.t - lambda.true$lambda.t
delta.positive <- delta.lambda
...
par(mar = c(0, 0.3, 0, 0.1))
plot(0, 0,
  xlim = c(0, my.xlim), ylim = c(-12, 8), type = "n",
  bty = "n", xlab = "", ylab = "", xaxt = "n", yaxt = "n", axes = FALSE
)
delta.lambda <- lambda.m3 - lambda.true$lambda.t
delta.positive <- delta.lambda
...
clean_sim_data_m2 <- cleanSimulationData(
  raw_data = sim_model2_data[[1]],
  cut_off = cut_off,
  N = length(object_par$f_vec_1)
)
...
#########
model1_par_est <- list(
  lambda0 = mean(sim_model2_stan_sim1_total[[plot.s]]$lambda0),
...
model2_par_est <- list(
  gamma = apply(
    sim_model2_stan_sim2_total[[plot.s]]$gamma,
    2, mean
  ),
...
To be updated with the correct paths, etc

```{r data for fig 5,cache=TRUE, eval=FALSE}
n_sim <- 50
num_nodes <- 10

object_par <- list(f_vec_1 = seq(from = 0.2, to = 0.9,
                                 length.out = num_nodes))
...
New potential Figure 5a plots. Need to improve this figure based on the newly
chosen simulation parameters above.

```{r log-p-value, eval=FALSE}
cols_plot <- col_df %>% 
...
all_ks %>% 
  filter(ks != 0) %>% 
  ggplot(aes(Method, ks, fill = Method)) + 
  geom_boxplot() +
  scale_fill_manual(values = cols_plot) +
  labs(y = "KS Statistic") +
  theme_minimal()
...
Inferred ranking boxplots against true rankings.


# Figure 5b: Simulation Spearman Rank

Need to update this again for the final simulation parameter setting.

```{r fig-5b-spearman-sim, fig.height=5}
sim_files <- list.files(here("output",
                             "revisions",
                             "aug_run",
                             "sim_rank/"), 
...
sim_data$name <- factor(sim_data$name, levels = c(
  "I&SI",
  "AggRank",
  "Glicko",
  "C-HP",
...
plot_cols <- col_df %>%
  filter(method %in% levels(sim_data$name)) %>%
  pull(cols)
...
## Figure 6a: Rank vs I&SI Cohort 5


```{r fig-6a-rank-vs-isi-cohort5}
...
f_draws <- sim_cohort_hp %>% select(starts_with("f"))
plot_latent_df$force[(1:(1000 * mice_number)) +
                       1000 * mice_number] <- as.vector(as.matrix(f_draws))

## M2
f_draws <- sim_cohort_dchp %>% select(starts_with("f"))
plot_latent_df$force[(1:(1000 * mice_number)) + 
                       2000 * mice_number] <- as.vector(as.matrix(f_draws))

## M3
f_draws <- sim_cohort_mmhp %>% select(starts_with("f"))
plot_latent_df$force[(1:(1000 * mice_number)) + 
                       3000 * mice_number] <- as.vector(as.matrix(f_draws))
...
## Figure 6b: Rank vs I&SI Cohort 3

```{r fig-6b-rank-vs-isi-cohort3}
### Rank Comparison for Single Cohort ####
current_cohort <- 3
# want to update this now
# data_path 
...
```{r fig-6b-overall-rank-correlation, eval=FALSE}
load("../output/revisions/aug_run/weighted_rank_data.RData")
# png(paste(plot_path,"real_rank_all_cohort.png",sep=""),width=600,height=300)
no_method <- 4
plot_cor_df <- data.frame(
  Method = rep(c(1:no_method), each = 10),
...
# Further results on latent ranks

Wish to investigate some further properties of the
latent states for these animals, in the context of the above latent rank
plot.

```{r prop-active-inactive, eval=FALSE}
### think need to run this
current_cohort <- 6
clean_data <- cleanData(full_data[[cohort_names[current_cohort]]])
return_df <- cleanObservationPeriod(current_cohort,
  raw_df =
...
### then look at these pairwise
length(interpolation_array_list)

dim(interpolation_array_list[[1]][[1]]) # each column here is a simulation
num_pairs <- length(interpolation_array_list)
num_winds <- length(interpolation_array_list[[4]])

## 500 for each window

mean_state <- matrix(NA, nrow = num_pairs, ncol = num_winds)

for(i in seq_along(1:num_pairs)) {
  for(j in seq_along(1:num_winds)) {
    if(i != 19) { # self directed event, remove
      curr_mean_vec <- apply(interpolation_array_list[[i]][[j]],
                                    1, mean)
      mean_state[i,j] <- mean(curr_mean_vec)
    }
  }
}
# apply(interpolation_array_list[[i]][[j]], 1, mean)
# dim(interpolation_array_list[[i]][[j]])

# then just take mean state, multiply by length of the window...
dim(unique_observe_win)


### this is the expected amount of time in the active state?
prop_active <- mean_state %*% 
  unique_observe_win$observe.time# /sum(unique_observe_win$observe.time)

prop_active[is.na(prop_active)] <- 0

pairs_tibble <- unique_pairs_df
pairs_tibble$prop_active <- prop_active

init_act <- pairs_tibble %>% group_by(initiator) %>% 
  summarise(mean_active = sum(prop_active)) %>% 
  arrange(desc(mean_active))

rec_act <- pairs_tibble %>% group_by(recipient) %>% 
  summarise(mean_active = sum(prop_active)) %>% 
  arrange(desc(mean_active))

bind_rows(init_act %>% rename(start = initiator),
          rec_act %>% rename(start = recipient)) %>% 
  group_by(start) %>% 
  summarise(total_active = sum(mean_active)) %>% 
  arrange(total_active)

```


```{r rerun-isi, eval=FALSE}
num_tries <- 100 
result <- matrix(NA, nrow = num_tries, ncol = 12)
for(i in 1:num_tries){
  print(i)
  raw_data <- full_data[[cohort_names[current_cohort]]]
  win_lost_df <- raw_data[-which(raw_data[,
                                        "Actor"]=="Start"|raw_data[,
                                                              "Actor"]=="End"),
                          c("Actor","Recipient")]
  win_lost_matrix <- get_wl_matrix(win_lost_df)
  sample
  # problem is that the initial rank the same the whole time.
  win_lost_matrix
  out <- isi98(win_lost_matrix[gtools::permute(1:12), gtools::permute(1:12)])
  a <- as.numeric(out$best_order)
...
```{r explore-cohort, eval=FALSE}
unique_pairs_df %>% 
  group_by(initiator) %>% 
  summarise(start = sum(count)) %>% 
  arrange(start)

unique_pairs_df %>% 
  group_by(recipient) %>% 
  summarise(receive = sum(count)) %>% 
  arrange(receive)

### look at out degree estimates also
sim_cohort_mmhp %>% 
  select(starts_with("gamma")) %>% 
...
### get sd for each of the 3 models
...
bind_rows(m1, m2, m3) %>% 
  left_join(m3, by = "name") %>% 
  rename(mmhp_var = value.y, var = value.x) %>% 
  select(-model.y) %>% 
  group_by(name) %>%
  summarise(per_var = var/mmhp_var, model = model.x) %>% 
  filter(model != "C-MMHP") %>% 
  # ungroup() %>% 
  # pull(per_var)
  mutate(per_var_cmmhp = scales::percent(per_var)) %>% 
  select(-per_var)
```

## Further rank information


```{r compute-isi-using-cmmhp-ranking, eval=FALSE}
f_med <- sim_cohort_mmhp %>% 
  select(starts_with("f")) %>% 
  summarise(across(everything(), median)) %>% 
  pivot_longer(everything())

isi_ord <- order(f_med$value,decreasing = TRUE)

df <- win_lost_df
df[, 1] <- factor(df[, 1], levels = 1:12)
df[, 2] <- factor(df[, 2], levels = 1:12)
df$result <- 1
m1 <- stats::xtabs(result ~ ., data = df)

result <- isi98(m1[isi_ord,isi_ord])
```

```{r investigate-bursty, eval=FALSE}
# inter_8_10 <- 
...
return_df %>% 
  ...
  select(initiator, 
         ...
         observe.end,
         event.times) %>% 
  unnest_longer(event.times) %>% 
  group_by(observe.id, initiator) %>% 
  arrange(event.times, .by_group = TRUE) %>% 
  mutate(inter = event.times - lag(event.times, default = 0)) %>% 
  ggplot(aes(inter)) + geom_histogram() +
  labs(title = "Fights from animal 12")
...
return_df %>% 
  ...
  mutate(inter = event.times - lag(event.times, default = 0)) %>% 
  ggplot(aes(inter)) + geom_histogram() +
  labs(title = "Fights to animal 12")



return_df %>% 
  filter(initiator %in% c(3) | recipient == 3) %>% 
  select(initiator, 
         recipient,
         observe.id,
         observe.start,
  ...
  mutate(inter = event.times - lag(event.times, default = 0)) %>% 
  ggplot(aes(inter)) + geom_histogram() + 
  labs(title = "Top Ranked Animals")

```


```{r investigate-animal-12, eval=FALSE}
### want to try establish what proportion of its fights occur in inactive
### periods compared to other animals, etc
cur_node <- 12


pairs <- unique_pairs_df %>% 
  ungroup() %>% 
  mutate(index = row_number()) %>% 
  filter(initiator == cur_node) %>% 
  select(index, initiator, recipient, observe) %>%
  # observe gives the window its in
  # unnest_wider(observe) %>%
  pull(index)

observe_winds <- unique_pairs_df %>% 
  ungroup() %>% 
  mutate(index = row_number()) %>% 
  filter(initiator == cur_node) %>% 
  select(index, initiator, recipient, observe)

## need to somehow get the corresponding entries of the matrix 
## for these
for(i in seq_along(1:nrow(observe_winds))) {
  curr_winds <- observe_winds$observe[[i]]
  print(mean_state[pairs, ][i, curr_winds])
}
...
###
### this is the mean state across all observation windows, compared
### to the mean state for windows where events are observed above
apply(mean_state[pairs, ], 1, mean)

###  
act_vec <- mean_state %*% 
  unique_observe_win$observe.time
act_vec[pairs] /sum(unique_observe_win$observe.time)
```

### Explore Latent States

Look at fights from/to a given animal and plot the inferred latent 
states for each window.

```{r plot-latent-given-animal, eval=FALSE}
# dim(interpolation_array_list[[1]][[1]])
# ### pair then window then segment times simulation
# 
# interpolation_array_list[[1]][[1]][1 , ] %>% 
#   map_dbl(., mean)
# 
# ### need a function to take a given
# 
# ### this is what I want
# dim(interpolation_array_list[[1]][[1]])
# apply(interpolation_array_list[[1]][[1]], 1, mean) 
# # want to do this for each pair

mean_states <- function(df) {
  return(apply(df, 1, mean))
}

pair_list <- unique_pairs_df %>% 
  ungroup() %>% 
  mutate(index = row_number()) %>% 
  filter(initiator == 12) %>% 
  pull(index)
...
for(i in seq_along(1:num_pairs)) {
  if(i != 19) {
    ...
    map(~mean_states((.x))) %>% 
    set_names(paste0("window_", 1:34)) %>% 
    as_tibble()
  }
}

pair_result # has for each pair in each element
saveRDS(pair_result, file = "pair_result.RDS")
```


Look at the output from this process, plot these, etc.

```{r look-at-pair-result, eval=FALSE}
pair_result <- readRDS("../pair_result.RDS")
theme_set(theme_minimal())

### need to get the event times within windows also...
# return_df %>% 
#   select(observe.id, initiator, recipient, event.times)
# 
# unique_observe_win
# ## this is everything I need for the plots then...
# unique_pairs_df %>% 
#   ungroup() %>% 
#   mutate(index = row_number()) %>% 
...
We can then look at some of these for some pairs of interest.
...
Similarly, we can look at the breakdown of events that are classified as
active as opposed to inactive.

```{r classify-events, eval=FALSE}
...
### compute isi for each
names(active_matrix) <- 1:12
colnames(active_matrix) <- 1:12
rownames(active_matrix) <- 1:12
isi98(active_matrix)

names(inact_matrix) <- 1:12
colnames(inact_matrix) <- 1:12
rownames(inact_matrix) <- 1:12
isi98(inact_matrix)
```



# Previous Figure 7: Pearson residual matrices

No longer including this figure in the paper
...
for (current_cohort in 1:10) {
  ...
  m3_score <- positiveNegativeApproxError(m3_pr) 
  ## smallest value which is good
...
#                                              "C-MMHP"))) %>%
struct_plot$Method <- factor(struct_plot$Method, levels = c(
  "I-MMHP", "C-HP", "C-DCHP",
  "C-MMHP"
))
...
```{r fig-8a-mae-norm-cohort5,eval=TRUE}
load("../output/revisions/aug_run/plot_N_predict_revision.RData")
...
```{r fig-7a-pred-all-cohorts-box-mae,eval=TRUE}
### updated to revision
load("../output/revisions/aug_run/plot_N_predict_revision.RData")
...
load("../output/revisions/aug_run/plot_rank_predict.RData")
...
```{r process data for plot 10b, cache=FALSE}
library(PlayerRatings)
current_cohort <- 5
# current_cohort <- 1
load(paste(data_path, cohort_names[current_cohort],
...
sub.types <- c("All", "Active", "Inactive")
# three methods
...
df1 <- df1[order(df1$Timestamp), ] # ensure in date order
df1$event <- 1:nrow(df1)
glick.df <- df1[, c("event", "Actor", "Recipient", "score"),
...
dsnl_x_sims <- sim_dsnl %>% select(starts_with("x"))

dsnl_arr <- array(as.matrix(dsnl_x_sims), dim = c(1000, 21, 12))

## All aggregate
for (t in 1:length(task.types)) {
  temp_vec <- rep(NA, 1000)
  cur_real_rank <- real.gl$history[, day_end_idx[t], 1]
  for (s in c(1:1000)) {
    temp_result <- cor.test(dsnl_arr[s, t, ], cur_real_rank,
      method = "spearman"
    )
    temp_vec[s] <- temp_result$estimate
  }
  data.tbl$sub.type[(1:1000) + (t - 1) * 1000] <- sub.types[1]
  data.tbl$task.type[(1:1000) + (t - 1) * 1000] <- task.types[t]
  data.tbl$data.value[(1:1000) + (t - 1) * 1000] <- temp_vec
}

dsnl_act_sims <- sim_dsnl_active %>% select(starts_with("x"))
dsnl_act_arr <- array(as.matrix(dsnl_act_sims), dim = c(1000, 21, 12))
...
for (t in 1:length(task.types)) {
  ...
  cur_real_rank <- real.gl$history[, day_end_idx[t], 1]
  for (s in c(1:1000)) {
    temp_result <- cor.test(-dsnl_act_arr[s, t, ], cur_real_rank,
                            method = "spearman")
    temp_vec[s] <- temp_result$estimate
  }
  data.tbl$sub.type[(1:1000) + (t - 1) * 1000 + 21000] <- sub.types[2]
  data.tbl$task.type[(1:1000) + (t - 1) * 1000 + 21000] <- task.types[t]
  data.tbl$data.value[(1:1000) + (t - 1) * 1000 + 21000] <- temp_vec
}

dsnl_inac_sims <- sim_dsnl_inactive %>% select(starts_with("x"))
dsnl_inac_arr <- array(as.matrix(dsnl_inac_sims), dim = c(1000, 21, 12))
...
for (t in 1:length(task.types)) {
  ...
  cur_real_rank <- real.gl$history[, day_end_idx[t], 1]
  for (s in c(1:1000)) {
    temp_result <- cor.test(dsnl_inac_arr[s, t, ],
      cur_real_rank,
      method = "spearman"
    )
    temp_vec[s] <- temp_result$estimate
  }
  data.tbl$sub.type[(1:1000) + (t - 1) * 1000 + 21000 * 2] <- sub.types[3]
  data.tbl$task.type[(1:1000) + (t - 1) * 1000 + 21000 * 2] <- task.types[t]
  data.tbl$data.value[(1:1000) + (t - 1) * 1000 + 21000 * 2] <- temp_vec
}
...
plot(
  x = 0, y = 0, xlim = x.lim, ylim = y.lim, xaxt = "n",
  col = "white", xlab = x.ttl,
  ylab = y.ttl, main = ttl,
  cex.main = cx * 1.3, cex.axis = cx, cex.lab = cx
)
axis(
  side = 1, at = 1:length(task.types), labels = task.types,
  cex.axis = cx, cex.lab = cx
) # put on the x-axis labels
...
for (t.id in 1:length(task.types)) {
  for (i in 1:length(sub.types)) { # t.id <- 1; i <- 1;
    ...
    boxplot(data.tbl$data.value[inds],
      at = t.id + shifts[i],
      col = cols[i], add = TRUE,
...
legend(
  x = "top", legend = sub.types, fill = cols,

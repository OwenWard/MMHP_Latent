...
knitr::opts_chunk$set(
  echo = TRUE, fig.path = "paper_figure/",
...
Plot the negative log of the p values here.

```{r potential-fig3-diag, eval=FALSE}
M1_ks_p <- tibble(ks_p = as.vector(ks_chp), method = "C-HP")

M2_ks_p <- tibble(ks_p = as.vector(ks_dchp), 
                  method = "C-DCHP")
...
Could repeat this across all cohorts?

What is `data_path` here?
...
```{r plot-pvalues-m1-m2-here}
ks_p %>% filter(ks_p != 0) %>%
  select(-cols) %>% 
  mutate(id = row_number()) %>% 
  pivot_wider(names_from = method, values_from = ks_p)
...
```{r fig-3b-rank-comparison, include=FALSE,eval=original_plots}
current_cohort <- 5
colors_plot <- col_df %>%
  filter(method %in% c("C-HP", "C-DCHP")) %>%
  pull(cols)
...
plot(order(expert_rank_10[[current_cohort]]),
  apply(sim_cohort_hp$f, 2, mean),
  pch = 16, cex = 1.2,
  # col= "steelblue",
  col = colors_plot[1],
  ylab = "", xlab = "",
  xlim = c(1, 12.5), ylim = c(0, 1),
  xaxt = "n", yaxt = "n"
)
segments(order(expert_rank_10[[current_cohort]]),
  apply(sim_cohort_hp$f, 2, mean) -
    apply(sim_cohort_hp$f, 2, sd),
...
axis(BELOW <- 1, at = 1:12, 
     labels = expert_rank_10[[current_cohort]],
     cex.axis = 1.3,
     tck = -0.015)
# mtext(text=11, side=1, line=0.5, cex=1.3, at=6)

axis(LEFT <- 2,
  at = seq(0, 1, 0.2), labels = seq(0, 1, 0.2), las = HORIZONTAL <- 1,
...
par(mar = c(3, 3, 1, 1))
plot(as.vector(n_matrix[n_matrix > 0]),
     as.vector(ks.stat.matrix[n_matrix > 0]),
     xlab = "",
     ylab = "",
     pch = 16,
     col = "gray",
...
mgp.axis(1, mgp = c(0, 0.3, 0),
         at = seq(0, 120, 10),
         cex.axis = 1,
         lwd = 0,
         lwd.ticks = 0.1)
mgp.axis(2, mgp = c(0, 0.1, 0),
         at = seq(-0.2, 0.6, 0.2),
         cex.axis = 1,
         lwd = 0,
         lwd.ticks = 0.8)
...
```{r fig-4a, fig.height=3.5, fig.width=8, eval=original_plots}
...
```{r fig4a-new-hist, fig.height=3.5, fig.width=8, eval=original_plots}
...
m1 <- sim_model3_stan_sim1_total %>%
  ...
  unnest(cols = value) %>%
  mutate(
    node_id = rep(c("f1", "f2", "f3", "f4", "f5"), 50),
    method = "C-HP"
  )
...
```{r new-fig-4a, eval = revision_plots, message=FALSE, warning=FALSE}
...
object_par <- list(f_vec_1 = seq(from = 0.1, to = 0.9,
                                 length.out = num_nodes))
### TO DO
...
```{r fig5a-residual-ks, fig.height=7, eval=original_plots}
...
for (i in c(5:1)) {
  for (j in c(5:1)) {
    if (i == j) {
      ...
      PR <- c(
        KS_model1_array[, i, j], KS_model2_array[, i, j],
...
mtext(
  text = c(1:5), side = 2, line = 0.5, outer = TRUE, cex = 2, las = 2,
  at = c(0:4) / 5 + 0.1
)
mtext(
  text = c(5:1), side = 1, line = 1.5, outer = TRUE, cex = 2,
  at = c(0:4) / 5 + 0.1
)
...
sim_ranks_mean %>% 
  ...
  ggplot(aes(node, ranks, fill = model)) + 
  geom_boxplot() +
  scale_fill_manual(values = cols_plot, guide = 'none') +
  facet_wrap(~model) +
  geom_point(aes(node, true_rank, colour = "True Value"),
             show.legend = TRUE) +
...
```{r m1-m2-pr, fig.width=6}
current_cohort <- 5
m1_m2_plot <- list(m1_pr[
    rev(expert_rank_10[[current_cohort]]),
    expert_rank_10[[current_cohort]]
  ],
  m2_pr[
...
myMultiMatrixPlot(
  X = m1_m2_plot, no_matrix = 2, n_row = 1,
  xLabels = expert_rank_10[[current_cohort]],
  yLabels = rev(expert_rank_10[[current_cohort]]),
  min = -80, max = 80, axis_cex = 2, title_cex = 1.8,
  colorPalette = "RdBu", if.emp = FALSE,
  # legend.mar=c(0.5,.5,0.5,0.5),
  title_lst = list("C-HP", "C-DCHP"),
  # title_lst = list("I-MMHP"),

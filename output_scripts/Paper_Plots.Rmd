...
Will need to update this with full final simulations.
...
sims_id <- 1:50
sims_id <- sims_id[c(-22,-23, -24, -33, -34, -36, -39, -40, -41, -45)]

for(curr_sim in sims_id) {
...
true_ranks <- tibble(node = 1:num_nodes, true_rank = f_vec)
...
Need to redo this after finally correctly formulating the 
simulations.

```{r prepare data for 4b,cache=TRUE, eval=revision_plots}
...
for (s in c(1:n_sim)) {
  ...
  model2_par_est <- list(
    ...
    ### TO DO
    gamma = apply(sim_model3_stan_sim2$gamma, 2, mean),
...
plot(1,
  type = "n", xlab = "", ylab = "Glicko ranking", xlim = c(0, dim(gl$history)[2]),

...
```{r fig-4a, fig.height=4,fig.width=8}
...
```{r fig4b-recover-latent-states,fig.width=8,fig.height=4}
...
#########
model1_par_est <- list(lambda0 =
                         mean(sim_model2_stan_sim1_total[[plot.s]]$lambda0[1001:2000]),
                       eta_1=mean(sim_model2_stan_sim1_total[[plot.s]]$eta_1[1001:2000]),
                       eta_2=mean(sim_model2_stan_sim1_total[[plot.s]]$eta_2[1001:2000]),
                       eta_3=mean(sim_model2_stan_sim1_total[[plot.s]]$eta_3[1001:2000]),
                       beta=mean(sim_model2_stan_sim1_total[[plot.s]]$beta[1001:2000]),
                       f=apply(sim_model2_stan_sim1_total[[plot.s]]$f[1001:2000,],2,mean))
lambda.m1<-uniHawkesIntensityNumeric(object=list(lambda0 = 
                                                   model1_par_est$lambda0,
                                    alpha =
                                      model1_fn$alpha.fun(model1_par_est$f[plot.i],
...
## model2
#############
model2_par_est <- list(gamma=apply(sim_model2_stan_sim2_total[[plot.s]]$gamma[1001:2000,],
                                   2,mean),
                    zeta = 
                      apply(sim_model2_stan_sim2_total[[plot.s]]$zeta[1001:2000,],2,mean),
                       eta_1=mean(sim_model2_stan_sim2_total[[plot.s]]$eta_1[1001:2000]),
                       eta_2=mean(sim_model2_stan_sim2_total[[plot.s]]$eta_2[1001:2000]),
                       eta_3=mean(sim_model2_stan_sim2_total[[plot.s]]$eta_3[1001:2000]),
                       beta=mean(sim_model2_stan_sim2_total[[plot.s]]$beta[1001:2000]),
                       f=apply(sim_model2_stan_sim2_total[[plot.s]]$f[1001:2000,],2,mean))
...
## Model3
#############
mmhp_par_est <- list(lambda0=mean(sim_model2_stan_sim3_total[[plot.s]]$lambda0[1001:2000]),
                     lambda1=mean(sim_model2_stan_sim3_total[[plot.s]]$lambda1[1001:2000]),
                     eta_1=mean(sim_model2_stan_sim3_total[[plot.s]]$eta_1[1001:2000]),
                     eta_2=mean(sim_model2_stan_sim3_total[[plot.s]]$eta_2[1001:2000]),
                     eta_3=mean(sim_model2_stan_sim3_total[[plot.s]]$eta_3[1001:2000]),
                     beta=mean(sim_model2_stan_sim3_total[[plot.s]]$beta[1001:2000]),
                     f=apply(sim_model2_stan_sim3_total[[plot.s]]$f[1001:2000,],2,mean))
...
sim_data %>% 
  ...
  ggplot(aes(Method,corr)) + geom_boxplot(aes(fill = Method)) +
...
struct_plot %>% 
  ggplot(aes(Method,Score)) + geom_boxplot() +
  facet_wrap(~Pos_Neg, scales = "free") + theme_minimal()
...
```{r fig-8a-frob-norm-cohort5, fig.width=8,fig.height=4}
...
```{r fig-8b-frob-norm-all-cohorts,fig.width=8,fig.height=4}
load("../output/plot_N_predict.RData")
...
p4 <- ggplot(temp_plot_df, aes(x=day, y=avg, group=method, color=method)) + 
  geom_line(aes(linetype = method),
            size=1.2, alpha=0.8, position=position_dodge(0.2)) +
  geom_errorbar(aes(ymin=avg-sd, ymax=avg+sd), width=0.8, size=0.8, alpha=0.8,
                 position=position_dodge(0.2)) +
  scale_color_manual(values = viridis::viridis(5)[1:5]) + 
  ggtitle("(b)") +
  theme_bw() +
  theme(plot.title = element_text(size = 22, hjust = 0.5, face="bold"),
        text = element_text(size = 20),

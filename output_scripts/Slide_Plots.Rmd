...
knitr::opts_chunk$set(echo = TRUE, fig.path = "slide_figure/",
                      dev = 'png', fig.height = 4.5, fig.width = 7,
                      warning = FALSE, eval = FALSE, include = FALSE)
...
```{r load rankings,echo=FALSE,eval=TRUE,results='hide',cache=TRUE}
...
model1_par_est <- list(lambda0 = mean(sim_model3_stan_sim1_total[[plot.s]]$lambda0),
                       eta_1 = mean(sim_model3_stan_sim1_total[[plot.s]]$eta_1),
                       eta_2 = mean(sim_model3_stan_sim1_total[[plot.s]]$eta_2),
                       eta_3 = mean(sim_model3_stan_sim1_total[[plot.s]]$eta_3),
...
```{r fig-5b-spearman-sim, fig.height=3, fig.width=6, include=TRUE,eval=TRUE}
...
plot_cols_slide <- col_df %>%
  filter(method %in% levels(sim_data$name)) %>%
  filter(method != "AggRank") %>%
  pull(cols)
...
sim_data %>%
  ...
  scale_fill_manual(values = plot_cols_slide) +
...
```{r fig-6a-rank-vs-isi, include=TRUE, eval=TRUE}
...
plot_latent_df$force[(1:(1000*mice_number))] <-
  as.vector(sim_cohort_hp$f)

## M2
plot_latent_df$force[(1:(1000*mice_number)) + 
                       1000*mice_number] <- as.vector(sim_cohort_dchp$f)

## M3
plot_latent_df$force[(1:(1000*mice_number)) + 
                       2000*mice_number] <- as.vector(sim_cohort_mmhp$f)
...
```{r fig-6b-overall-rank-correlation, include=TRUE, eval=TRUE}
...
cols_here <- col_df %>% 
  filter(method %in% c("AggRank","C-HP","C-DCHP","C-MMHP")) %>%
  pull(cols)

p2 <- ggplot(plot_cor_df, aes(x="", y = cor, fill = Method)) +
        geom_boxplot(alpha=0.7, width=0.5) +
        scale_fill_manual(values = cols_here) + 
        # ggtitle("(b)") +
        ylim(0,1) +
        theme_bw() +
        theme(plot.title = element_text(size = 22, hjust = 0.5, face="bold"),
...
```{r fig-7-prMatrix-Cohort3, fig.width=7, fig.height=3, eval=TRUE, include=TRUE}
current_cohort <- 3
...
plot_cols_pred <- col_df %>% filter(method %in% levels(temp_plot_df$method)) %>%
...
```{r fig-8b-frob-norm-all-cohorts,fig.width=8,fig.height=4.5, eval=FALSE}
load("../output/plot_N_predict.RData")
...
#png(paste(plot_path,"real_predict_N_all_cohort.png",
# sep=""),width=600,height=300)
p4 <- ggplot(temp_plot_df, aes(x=day, y=avg, group=method, color=method)) + 
  ...
  scale_color_manual(values = plot_cols_pred) + 
  ggtitle("(b)") +
  theme_bw() +
  theme(plot.title = element_text(size = 22, hjust = 0.5, face="bold"),
        text = element_text(size = 20),
...
```{r fig-7a-pred-all-cohorts-box, include=TRUE, eval=TRUE, fig.width=7,fig.height=3}
load("../output/plot_N_predict.RData")
...
```{r fig-7b-pred-rank-spearman-all-cohorts, include=TRUE, eval=TRUE, fig.width=7,fig.height=3}
...
```{r construct data for plot, results='hide',message=FALSE,cache=TRUE,eval=TRUE}
...
```{r fig-10a-glicko-rating,include=TRUE,eval=TRUE}
par(mar=c(3,5,3,1))

...
Examing the correlation matrices of both the median ranks for
...
Finally, for each animal in this cohort, we summarise
...
cmmhp_gamma %>% 
  ...
  geom_text(data = out_perc, aes(name, max_est + 0.25, label = perc_out),
            size = 2) +
  # theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
...
cmmhp_zeta %>% 
  ...
  scale_fill_manual(values = "#FDE725FF") +
  facet_wrap(~cohort, scales = "free", ncol = 2) +
  geom_text(data = in_perc, aes(name, max_est + 0.25, label = perc_in),
            size = 2) +
  # theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
